...
class DataLab(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.user_name.setText('欢迎使用收银员人员管理')
        ...
        self.btn1 = QPushButton('增加收银员', self)
        self.btn1.setToolTip('收银员数据增加')
        self.btn1.resize(220, 60)
        self.btn1.move(480, 820)

        self.btn2 = QPushButton('删除收银员', self)
        self.btn2.setToolTip('收银员数据删除')
    ...
    def buttonClicked(self):
        ...
        if sender.text() == str('增加收银员'):
            self.onadd()
        elif sender.text() == str('删除收银员'):
            # 后面四个数字的作用依次是 初始值 最小值 最大值 步幅
            value, ok = QInputDialog.getText(self, "删除收银员", "请输入你要删除的收银员员id：")
            self.del_date(value)
    ...
    def del_date(self, id):
        ...
        sql_delete = "DELETE FROM cashier WHERE id = %d;" \
                     % int(id)
        adface_delete = "DELETE FROM cashier_face WHERE name_id = %d;" \
                        % int(id)
        cursor = conn.cursor()
        # 定义要执行的SQL语
...
class add_win(QWidget):
    ...
    def addNum(self):
        ...
        self.add_face = cashier_FaceCollect.GUI(name, id)
        self.add_face.show()
        self.close()
    ...
    def add_sql(self, id, name, password):
        ...
        sql_insert = "INSERT INTO cashier VALUES (%d, %s, %s, %s);" \
...
#         self.face_col = cashier_FaceCollect.GUI(name, id)
#         self.addwin.close()
#         self.face_col.show()
